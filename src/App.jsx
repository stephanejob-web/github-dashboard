...
export default function App() {
  ...
  async function handleSearch(owner, repo, token) {
    setCurrent({ owner, repo, token })
    try { sessionStorage.setItem(LAST_SEARCH_KEY, JSON.stringify({ owner, repo })) } catch {}
    await load(owner, repo, token)
  }
    ...
    return (
      <div style={{ minHeight: '100vh', background: 'var(--navy)', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 24, padding: 32, position: 'relative', overflow: 'hidden' }}>
        ...
        <div style={{ position: 'relative', width: 72, height: 72 }}>
          <div style={{
            width: 72, height: 72, borderRadius: '50%', border: '3px solid rgba(255,255,255,0.06)',
            borderTopColor: '#4318ff', borderRightColor: '#7551ff',
            animation: 'spin 1s linear infinite',
          }} />
          <div style={{
            position: 'absolute', inset: 8, borderRadius: '50%',
            background: 'linear-gradient(135deg,rgba(66,42,251,0.2),rgba(1,181,116,0.1))',
          }} />
        </div>
        ...
        <div style={{ width: '100%', maxWidth: 480, position: 'relative', zIndex: 1 }}>
          ...
          <div style={{ textAlign: 'center', marginBottom: 32 }}>
            <div style={{
              ...
              background: isRateLimit ? 'linear-gradient(135deg,#e31a80,#f97316)' : 'linear-gradient(135deg,#f97316,#dc2626)',
              display: 'flex', alignItems: 'center', justifyContent: 'center',
              boxShadow: '0 0 50px rgba(227,26,128,0.45)',
            }}>
            ...
            <p style={{ fontSize: 13, color: '#718096' }}>
              {isRateLimit
                ? '60 req/h sans token · 5000 req/h avec token GitHub'
                : isNotFound
                  ? 'Vérifie le nom du repository (ex: torvalds/linux) et qu\'il est bien public.'
                  : error}
            </p>
          </div>
          ...
          {isRateLimit && (
            <div className="glass" style={{ padding: 24, marginBottom: 16 }}>
              <p style={{ fontSize: 13, fontWeight: 700, color: '#868cff', marginBottom: 20, textTransform: 'uppercase', letterSpacing: '0.08em' }}>
                Créer un token GitHub gratuit
              </p>
                ...
                {steps.map(s => (
                  <div key={s.num} style={{ display: 'flex', gap: 16, alignItems: 'flex-start' }}>
                    <div style={{
                      ...
                      boxShadow: '0 0 12px rgba(66,42,251,0.4)',
                    }}>{s.num}</div>
                    <p style={{ fontSize: 13, color: '#a0aec0', lineHeight: 1.6, paddingTop: 6 }}>
                      {s.text}{' '}
                      <strong style={{ color: '#e2e8f0' }}>{s.bold}</strong>
          ...
          <button onClick={() => { reset(); setCurrent(null) }} style={{
            ...
            boxShadow: '0 0 30px rgba(66,42,251,0.4)', transition: 'opacity 0.2s',
          }}
            onMouseEnter={e => e.currentTarget.style.opacity = '0.85'}
            onMouseLeave={e => e.currentTarget.style.opacity = '1'}
          >
            {isRateLimit ? '→ Réessayer avec un token' : 'Retour'}
          </button>
        </div>
      </div>
    )
  ...
  return <RepoSearch onSearch={handleSearch} loading={loading} />
}
